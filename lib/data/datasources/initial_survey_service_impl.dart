...
class InitialSurveyServiceImpl extends APIServiceBase
    implements InitialSurveyService {
  ...
  Future<APIResponse<List<InitialSurveyQuestion>>> getInitialSurvey() =>
      get<List<InitialSurveyQuestion>>(
          '/api/initialsurvey',
          (dynamic items) => items
              .map<InitialSurveyQuestion>(
                  (e) => InitialSurveyQuestion.fromJson(e))
              .toList());

  @override
  Future<APIResponse> submit(List<InitialSurveyQuestionResponse> response) {
    return postMany(
        '/api/respondents', response.map((e) => e.toJson()).toList());
  }

  @override
  Future<APIResponse<Map<String, dynamic>>> getMyResponse() =>
      get<Map<String, dynamic>>('/api/respondents', (dynamic json) => json);
}
